...
import sys
sys.path.append( '/home/shinan/catkin_ws/src/Yolov5s/scripts/yolov5')
import torch
...
from yolo_v5s.msg import BoundingBox, BoundingBoxes
...
package = RosPack()
package_path = package.get_path('yolo_v5s')

class DetectorManager():
    def __init__(self):
        # Load weights parameter
        weights_name = rospy.get_param('~weights_name', '/home/shinan/catkin_ws/src/Yolov5s/scripts/yolov5/yolov5s.pt')
    ...
    #----------------------------------------------------------------------------add yolov5
        ...
        # Define subscribers
        self.image_sub = rospy.Subscriber(self.image_topic, Image, self.image_callback, queue_size=1, buff_size=2 ** 32)

        # Define publishers
        self.pub_ = rospy.Publisher(self.detected_objects_topic, BoundingBoxes, queue_size=1)
    ...
    def image_callback(self, data):
        ...
        print('预处理运行时间:%s毫秒' % ((T2 - T1)*1000))
        print('模型计算运行时间:%s毫秒' % ((T3 - T2)*1000))
        print('后处理运行时间:%s毫秒' % ((T4 - T3)*1000))
        # Visualize detection results
        if (self.publish_image):
            self.visualizeAndPublish(detection_results, self.cv_image)
        # print('%s' %detection_results.header)
        return True
    ...
    def imagePreProcessing(self, img):
        ...
        input_img = cv2.resize(self.padded_image, (self.network_img_size, self.network_img_size), interpolation=cv2.INTER_CUBIC) / 255.
